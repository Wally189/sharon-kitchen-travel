...
const simpleLists = {
  ...
  storyHighlights: {
    ...
    placeholder: "Friendly booking process"
  },
  vehicleBullets: {
    containerId: "vehicleBulletsList",
    addId: "addVehicleBullet",
    placeholder: "Ideal for planned group travel"
  }
};
...
const objectLists = {
  services: {
    ...
    addId: "addService",
    fields: [
      { key: "title", label: "Title", placeholder: "Wedding transport" },
      { key: "summary", label: "Summary", type: "textarea", full: true, placeholder: "Describe the service." },
      { key: "anchorText", label: "Support line", placeholder: "For organised wedding-day travel" }
    ]
  },
  gallery: {
    ...
    fields: [
      { key: "image", label: "Image path", placeholder: "/assets/images/uploads/vehicle.jpg", full: true, isImage: true },
      { key: "title", label: "Title", placeholder: "Vehicle view" },
      { key: "caption", label: "Caption", type: "textarea", full: true, placeholder: "Describe this image." }
    ]
  },
  faq: {
    containerId: "faqList",
    addId: "addFaqItem",
    fields: [
      { key: "question", label: "Question", full: true, placeholder: "What kinds of bookings do you cover?" },
...
function renderHeader() {
  byId("dashboardTitle").textContent = state.site?.businessName || "Website Editor";
  byId("dashboardIntro").textContent = "Change website text or images and rebuild the static site.";
}
  ...
  images.forEach((item) => {
    ...
    card.innerHTML = `
      ...
      <div class="library-body">
        ...
        <p class="helper-text">${escapeHtml(item.note || "Available for hero, gallery or section images.")}</p>
...
function buildSiteTitle(businessName, tagline) {
  const safeBusinessName = businessName || state.site?.businessName || "Business name";
  const summary = tagline || state.site?.tagline || "Private hire transport";
...
function buildImageAlt(title, fallback, defaultValue) {
  return title || fallback || defaultValue;
}
